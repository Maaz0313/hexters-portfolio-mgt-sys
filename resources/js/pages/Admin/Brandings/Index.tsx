      ...
      <div className="py-12">
        <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold">Manage Brandings</h1>
            <Link
          ...
          <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div className="p-6 bg-white border-b border-gray-200">
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {brandings.data.length > 0 ? (
                      brandings.data.map((branding) => (
                        <tr key={branding.id}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">{branding.name}</div>
                          </td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-500">
                              {branding.portfolio_projects_count}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-500">
                              {format(new Date(branding.created_at), 'MMM d, yyyy')}
                            </div>
                          </td>
              ...
              {/* Pagination */}
              {brandings.last_page > 1 && (
                <div className="mt-4 flex justify-between items-center">
                  <div className="text-sm text-gray-500">
                    Showing {(brandings.current_page - 1) * brandings.per_page + 1} to{' '}
                    {Math.min(brandings.current_page * brandings.per_page, brandings.total)} of{' '}
                    {brandings.total} results
                  </div>
                  <div className="flex space-x-2">
                    {Array.from({ length: brandings.last_page }, (_, i) => i + 1).map((page) => (
                      <Link
                        key={page}
                        href={route('admin.brandings.index', { page })}
                        className={`px-3 py-1 rounded ${
                          page === brandings.current_page
