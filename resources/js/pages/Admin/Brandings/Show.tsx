...
const Show = ({ branding }: Props) => {
  const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
      title: branding.name,
      href: `/dashboard/brandings/${branding.id}`,
    },
  ];

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title={`Branding: ${branding.name}`} />
      <div className="py-12">
          ...
          <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div className="p-6 bg-white border-b border-gray-200">
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6">
                <h2 className="text-lg font-medium mb-2">Branding Details</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm font-medium text-gray-500">Name</p>
                    <p className="text-base">{branding.name}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Slug</p>
                    <p className="text-base">{branding.slug}</p>
                  </div>
                  <div className="md:col-span-2">
                    <p className="text-sm font-medium text-gray-500">Description</p>
                    <p className="text-base">
                      {branding.description || <span className="text-gray-400 italic">No description</span>}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Created At</p>
                    <p className="text-base">{format(new Date(branding.created_at), 'MMM d, yyyy h:mm a')}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Updated At</p>
                    <p className="text-base">{format(new Date(branding.updated_at), 'MMM d, yyyy h:mm a')}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Portfolio Projects</p>
                    <p className="text-base">{branding.portfolio_projects_count}</p>
                  </div>
                </div>
              </div>
            </div>
