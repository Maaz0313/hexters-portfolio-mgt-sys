...
const Portfolio = ({ portfolioProjects = [], brandings = [], industries = [], filters = { search: '', branding: '', industry: '' } }: Props) => {
  ...
  const projects: FormattedProject[] = Array.isArray(portfolioProjects) ? portfolioProjects.map(project => {
  ...
  const [currentProject, setCurrentProject] = useState<FormattedProject | null>(null);
  const [searchTerm, setSearchTerm] = useState(filters.search);
  const [selectedBrandings, setSelectedBrandings] = useState<string[]>(
    filters.branding ? filters.branding.split(',') : []
  );
  const [selectedIndustries, setSelectedIndustries] = useState<string[]>(
    filters.industry ? filters.industry.split(',') : []
  );
  ...
  const brandingItems = Array.isArray(brandings) ? brandings.map(branding => ({
    id: branding.id ? branding.id.toString() : '',
    label: branding.name || ''
  })) : [];

  // Format industry items for dropdown
  const industryItems = Array.isArray(industries) ? industries.map(industry => ({
        ...
        <div className="container mx-auto w-full max-w-[1220px] px-6 py-0">
          <div className="mb-48 text-center">
            ...
            <p className="text-[1.1428571429rem]/[1.5] font-light text-[#18161a] md:text-2xl/[1.375]">
              Luminary has produced over 400 successful projects over two decades
              in the industry. Here are just a few examples of our recent work.
            </p>
          </div>
          ...
          currentPosts.map((post, index) => (
            <div key={index}>
              <BlogPostCard
                ...
                onClick={() => handlePortfolioItemClick(post)}
              />
            </div>
          ))
        ) : (
